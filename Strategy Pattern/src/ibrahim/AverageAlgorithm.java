...
public class AverageAlgorithm implements Smoothing{
	...
	private int upLeftCornerFillArray(int x,int y,Image image){
		...
		for(int i = x; i < x + 3  ; i++){
			for(int j = y; j < y + 3; j++ ){
				sumPixelTotal = sumPixelTotal + image.getXYByte(i, j);			
			}
		}
		int newValue = sumPixelTotal / 9;
		return newValue;
	}
